
<!DOCTYPE html>
<html>

<head>
    <meta charset="utf-8">
    <meta name="robots" content="noindex">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Yearly: Report Builder</title>
 
    <!-- Styles -->
    <style>
      .teksTengah {
        display: flex;
        align-items: center;
        }
        .youtube-video {
          width: 90%;
        }
    </style>
    <link rel="stylesheet" href="https://use.typekit.net/ovg3oac.css">
    <link
        href="https://fonts.googleapis.com/css?family=Montserrat"
        rel="stylesheet">
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-9ndCyUaIbzAi2FUVXJi0CjmCapSmO7SnpJef0486qhLnuZ2cdeRhO02iuK6FUUVM" crossorigin="anonymous">
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/js/bootstrap.bundle.min.js" integrity="sha384-geWF76RCwLtnZ8qwWowPQNguL3RmwHVBC9FhGdlKrxdiJJigb/j/68SIy3Te4Bkz" crossorigin="anonymous"></script>
    
    <link rel="stylesheet" type="text/css" href="styles.css" />
    <script type="text/javascript" src="https://cdn.jsdelivr.net/npm/chartjs-plugin-datalabels"></script>
    <script defer src="https://use.fontawesome.com/releases/v5.8.1/js/all.js"
        integrity="sha384-g5uSoOSBd7KkhAMlnQILrecXvzst9TdC09/VM+pjDTCM+1il8RHz5fKANTFFb+gQ"
        crossorigin="anonymous"></script>
    <script src="https://code.jquery.com/jquery-3.3.1.min.js"
        integrity="sha256-FgpCb/KJQlLNfOu91ta32o/NMZxltwRo8QtmkMRdAu8=" crossorigin="anonymous">
        </script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/jqueryui/1.12.1/jquery-ui.min.js"></script>
    <script src="functions-print.js"></script>

    <script>
        window.twttr = (function (d, s, id) {
            var js, fjs = d.getElementsByTagName(s)[0],
                t = window.twttr || {};
            if (d.getElementById(id)) return t;
            js = d.createElement(s);
            js.id = id;
            js.src = "https://platform.twitter.com/widgets.js";
            fjs.parentNode.insertBefore(js, fjs);

            t._e = [];
            t.ready = function (f) {
                t._e.push(f);
            };

            return t;
        }(document, "script", "twitter-wjs"));
    </script>
</head>

<body data-view="publicReport">
    <nav id="reportNav">
        <div class="sidebar">
            <!-- <a class="logo" href="#"></a> -->

            <a style="text-decoration:none" class="toc" href="#" data-action="openDaftarIsi"><i class="fa fa-caret-down"></i>
            &nbsp;&nbsp;Daftar Isi&nbsp;&nbsp;</i></a>
            <!-- <a class="yearly" href="#">Yearly</a> -->
        </div>
        <div class="tableContents">
            <div>
                <ul></ul>
            </div>
        </div>
       
    </nav>
    <div id="publicReport">
    <div class="container">
      <div class="col-12" >
        <div class="row teksTengah"   style=" background-image: url('gambar/background1.png'); background-repeat: no-repeat;
                background-size: 100% 100%; min-height: 800px;">
            <div class="col-6" >
              <img width="100%" src="gambar/home1.png" alt="">
            </div>
            <div class="col-6">
              <img width="50%" src="gambar/home2.png" alt="">
              <img width="100%" src="gambar/judul.png" alt="">
            </div>
          </div>
      </div>
      <div class="col-12">
          <div class="row teksTengah"   style=" background-image: url('gambar/background2.png'); background-repeat: no-repeat;
                background-size: 100% 100%; min-height: 800px;  top: -905px;">
            <div class="col-6" >
                <h1 style="font-size: 9rem;" >01</h1>
                <h1 style="font-size: 7rem;">Ikhtisar Keuangan</h1>
                <h1 style="font-size: 3rem;">Financial Highlights</h1>
            </div>
            <div class="col-6">
            </div>
          </div>
      </div>
      <div class="col-12">
          <div class="row teksTengah"   style=" background-image: url('gambar/background3.png'); background-repeat: no-repeat;
                background-size: 100% 100%; min-height: 800px;">
            <div class="col-6" >
            </div>
            <div class="col-6">
                <h1 style="font-size: 35px; font-weight: bold; color:#06539A;" >Profil Dewan Komisaris</h1>
                <p style="font-size: 2rem;" >Board of Commissioners’ Profile</p>
                <p style="font-size: 11px;">PT BRI Asuransi Indonesia atau lebih dikenal dengan nama BRI Insurance atau BRINS selanjutnya disebut Perseroan adalah sebuah perusahaan asuransi umum nasional yang didirikan pada tanggal 17 April 1989 berdasarkan akta notaris Muhaini Salim, S.H. di Jakarta dengan No. 121 dan disahkan dengan penetapan Menteri Kehakiman RI pada tanggal 3 Mei 1989 No. C2.4160. HT.01.01 Tahun 1989. Perseroan memperoleh izin lisensi Dagang dari Menteri Keuangan RI dengan pengesahan No. KEP.128/KM.13/1989 tanggal 26 Agustus 1989. <br><br>

                    Tujuan awal didirikannya BRI Insurance adalah untuk memberikan perlindungan asuransi terhadap aset PT Bank Rakyat Indonesia (Persero), Tbk. termasuk agunan para nasabahnya. Seiring dengan perkembangannya, BRI Insurance terus memperluas pangsa pasar dan jaringan kerja yang tidak hanya berfokus kepada bank BRI dan nasabahnya, tetapi juga melayani masyarakat luas, baik itu perorangan maupun perusahaan, termasuk BUMN, BUMD, dan perusahaan swasta. Untuk melengkapi pelayanan Perseroan kepada masyarakat, pada tahun 2003, Unit Usaha Syariah didirikan untuk melayani pangsa pasar Syariah dan berencana melakukan spin off pada tahun 2024 sesuai dengan yang tercantum dalam Rencana Kerja Pemisahan Unit Syariah (RKPUS).<br><br>

                  Sejak tanggal 31 Januari 2020, PT Asuransi Bringin Sejahtera Artamakmur berganti nama menjadi PT BRI Asuransi Indonesia, perubahan nama perusahaan diiringi dengan rebranding logo dari BRINS menjadi BRI Insurance. Rebranding resmi diluncurkan pada saat BRI Insurance merayakan ulang tahun nya yang ke-31 (tiga puluh satu) pada tanggal 17 April 2020. Transformasi yang dilakukan merupakan salah satu wujud dari peralihan saham milik Dana Pensiun BRI kepada PT Bank Rakyat Indonesia (Persero), Tbk.</p>
                <p >Bambang Krisminarno*</p>
                <p>Komisaris Utama <br>
                  President Commissioner</p>
            </div>
          </div>
      </div>
      
      <div class="col-12">
          <div class="row teksTengah"   style=" 
                background-size: 100% 100%; min-height: 800px;">
            <div class="col-6" >
            <iframe class="youtube-video"  width="560" height="315" src="https://www.youtube.com/embed/zqvNe61x95U" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>
            </div>
            <div class="col-6">
                <h1 style="font-size: 2rem;" >Sekilas Perseroan</h1>
                <h1 style="font-size: 2rem;" >Company at A Glance</h1>
                <p style="font-size: 13px;">PT BRI Asuransi Indonesia atau lebih dikenal dengan nama BRI Insurance atau BRINS selanjutnya disebut Perseroan adalah sebuah perusahaan asuransi umum nasional yang didirikan pada tanggal 17 April 1989 berdasarkan akta notaris Muhaini Salim, S.H. di Jakarta dengan No. 121 dan disahkan dengan penetapan Menteri Kehakiman RI pada tanggal 3 Mei 1989 No. C2.4160. HT.01.01 Tahun 1989. Perseroan memperoleh izin lisensi Dagang dari Menteri Keuangan RI dengan pengesahan No. KEP.128/KM.13/1989 tanggal 26 Agustus 1989. <br><br>

                    Tujuan awal didirikannya BRI Insurance adalah untuk memberikan perlindungan asuransi terhadap aset PT Bank Rakyat Indonesia (Persero), Tbk. termasuk agunan para nasabahnya. Seiring dengan perkembangannya, BRI Insurance terus memperluas pangsa pasar dan jaringan kerja yang tidak hanya berfokus kepada bank BRI dan nasabahnya, tetapi juga melayani masyarakat luas, baik itu perorangan maupun perusahaan, termasuk BUMN, BUMD, dan perusahaan swasta. Untuk melengkapi pelayanan Perseroan kepada masyarakat, pada tahun 2003, Unit Usaha Syariah didirikan untuk melayani pangsa pasar Syariah dan berencana melakukan spin off pada tahun 2024 sesuai dengan yang tercantum dalam Rencana Kerja Pemisahan Unit Syariah (RKPUS).<br><br>

                  Sejak tanggal 31 Januari 2020, PT Asuransi Bringin Sejahtera Artamakmur berganti nama menjadi PT BRI Asuransi Indonesia, perubahan nama perusahaan diiringi dengan rebranding logo dari BRINS menjadi BRI Insurance. Rebranding resmi diluncurkan pada saat BRI Insurance merayakan ulang tahun nya yang ke-31 (tiga puluh satu) pada tanggal 17 April 2020. Transformasi yang dilakukan merupakan salah satu wujud dari peralihan saham milik Dana Pensiun BRI kepada PT Bank Rakyat Indonesia (Persero), Tbk.</p>
                
              
            </div>
          </div>
      </div>
      
    </div>
    <script>
        $(document).ready(function () {
            loadTemplate()
            $(document).on('click', 'a[data-action="openDaftarIsi"]', function (e) {
                $('#publicReport').toggleClass('active');
            });

            // Donate Click
            $(document).on('click', '[data-component="donate"] [data-href]', function () {
                var link = $(this).attr('data-href');
                window.open(link);
            });

            // Next Page
            $(document).on('click', '[data-action="nextPage"]', function (e) {
                $("iframe").each(function () {
                    var src = $(this).attr('src');
                    $(this).attr('src', src);
                });
                $(".instagram-media").each(function () {
                    var video = $(this);
                    var old = video.attr('src');
                    video.attr('src', '');
                    setTimeout(function () {
                        video.attr('src', old);
                    }, 0);
                });
                var nextElement = $(this).parents('article').next();

                // Set Page Marker
                var pageID = $(this).parents('article').next().attr('id');
                $('a[data-action="gotoPage"]').parents('li').removeClass('active');
                $('a[data-page="' + pageID + '"]').parents('li').addClass('active');

                // Close all other pages
                $('article.active').addClass('hide');

                setTimeout(function () {
                    $('article.active').removeClass('hide').removeClass('active');
                }, 600);


                // Open selected page
                setTimeout(function () {
                    $('article, html, body').scrollTop(0);
                    nextElement.addClass('active');
                }, 600);

                adjustWidth();
                e.preventDefault();
            });

            // Page Jump 
            $(document).on('click', 'a[data-action="gotoPage"]', function (e) {

                if (getParam('rid') == '-L_m9I0B6WFDXMYPt5t0') {
                    var position = ($(this).parents('li').index() + 1);
                    var nextElement = $('article:nth-of-type(' + position + ')');

                    $('#publicReport').toggleClass('active');

                    // Set pagemarker
                    $('a[data-action="gotoPage"]').parents('li').removeClass('active');
                    $(this).parents('li').addClass('active');

                    // Close all other pages

                    setTimeout(function () {
                        $('article.active').addClass('hide');
                    }, 500);

                    setTimeout(function () {
                        $('article.active').removeClass('hide').removeClass('active');
                    }, 1200);


                    // Open selected page
                    setTimeout(function () {
                        $('article, html, body').scrollTop(0);
                        nextElement.addClass('active');
                    }, 1200);


                } else {
                    var pageID = $(this).attr('data-page');
                    var nextElement = $('#' + pageID);

                    $('#publicReport').toggleClass('active');

                    // Set Page Marker
                    $('a[data-action="gotoPage"]').parents('li').removeClass('active');
                    $(this).parents('li').addClass('active');

                    // Close all other pages

                    setTimeout(function () {
                        $('article.active').addClass('hide');
                    }, 500);

                    setTimeout(function () {
                        $('article.active').removeClass('hide').removeClass('active');
                    }, 1200);


                    // Open selected page
                    setTimeout(function () {
                        $('article, html, body').scrollTop(0);
                        nextElement.addClass('active');
                    }, 1200);
                }

                adjustWidth();
                e.preventDefault();
            });



            function checkPosition() {

                var minPos = 1;
                var maxPos = $('article').length;
                var currentPage = $('article.active').attr('rel');

                console.log(currentPage);

                if (currentPage == minPos) {
                    $('[data-action="prevPage"]').attr('disabled', true);
                } else {
                    $('[data-action="prevPage"]').attr('disabled', false);
                }

                if (currentPage == maxPos) {
                    $('[data-action="nextPage"]').attr('disabled', true);
                } else {
                    $('[data-action="nextPage"]').attr('disabled', false);
                }

            }

            function adjustWidth() {
                if ($(window).width() <= 900) {
                    $('.fullwidth').parents('[data-columns]').css({
                        'min-height': '50vh'
                    });
                } else {
                }
            }
            function adjustVideosSize() {
                var videos = null;
                const interval = setInterval(() => {
                    videos = $('[data-component="video"] .container')
                    if (videos.length) {
                        clearInterval(interval)
                        videos.each(function () {
                            $(this).addClass("video-container")
                            var iframe = $(this).find('iframe')
                            iframe.addClass("video-iframe")
                            iframe.attr("style", "height: 100% !important")
                            var video = iframe[0]
                            var videoHeight = video.height;
                            var videoWidth = video.width;
                            var aspectRatio = 0
                            aspectRatio = (videoHeight / videoWidth) * 100;
                            $(this).css("padding-bottom", aspectRatio + "%")
                        });
                    }
                }, 500);
            }
            function adjustSocialPostSize() {
                var posts = null;
                const interval = setInterval(() => {
                    posts = $('[data-component="social"] .container')
                    if (posts.length) {
                        clearInterval(interval);
                        posts.each(function () {
                            //var parent = $( this ).closest( "[data-columns]" ).parent().attr( "style", "position: static !important" );
                            var iframe = $(this).find('iframe');
                            var post = iframe[0];
                            console.log('From adjustSocialPostSize post ' + JSON.strinfify(post));
                            const splitID = post.id.split('-');
                            console.log('From adjustSocialPostSize splitID ' + splitID);
                            if (splitID.indexOf('twitter') !== -1) {
                                //let message = { height: document.body.scrollHeight, width: document.body.scrollWidth };	
                                console.log('From adjustSocialPostSize post.dataset.tweetId ' + post.dataset.tweetId);
                                const elDiv = document.createElement('div');
                                elDiv.setAttribute('id', post.dataset.tweetId);
                                elDiv.style.width = "100%";

                                const elDivContainer = post.closest('div.container');

                                elDivContainer.appendChild(elDiv);
                                console.log(elDiv);
                                twttr.widgets.createTweet(
                                    post.dataset.tweetId,
                                    elDiv
                                )
                                post.remove();


                            }
                            else {
                                var postHeight = post.height;
                                console.log(post);
                                var postWidth = post.width;
                                iframe.attr("style", "height:" + postHeight + 20 + " !important; min-height:170px");
                                //post.attr( 'min-height', "170px" )
                            }

                        });
                    }
                }, 500);
            }
            function adjustParagraphsSize() {
                var posts = null;
                const interval = setInterval(() => {
                    posts = $('[data-component="paragraph"]')
                    if (posts.length) {
                        clearInterval(interval)
                        posts.each(function () {
                            var parent = $(this).closest('[data-columns="2full"]').parent().attr("style", "/*position: static !important*/")
                        });
                    }
                }, 500);
            }
            function adjustQouteSize() {
                var posts = null;
                const interval = setInterval(() => {
                    posts = $('[data-component="quote"]')
                    if (posts.length) {
                        clearInterval(interval)
                        posts.each(function () {
                            var parent = $(this).closest('[data-columns="2full"]').parent().attr("style", "position: absolute")
                        });
                    }
                }, 500);
            }
        });

        function loadTemplate() {
                loadShare()
        } 
        
  function loadShare() {
          $('#reportNav ul').append(
            '<li style="list-style:none;" class="active"><a style="text-decoration:none" href="#" data-action="gotoPage"><span>Iktisar Keuangan</span></a></li>' +
              '<li><a style="text-decoration:none" href="#" data-action="gotoPage"><span>Laporan Management</span></a></li>' +
              '<li><a style="text-decoration:none" href="#" data-action="gotoPage"><span>Profil Perusahaan </span></a></li>' +
              '<li><a style="text-decoration:none" href="#" data-action="gotoPage"><span>Tinjau Operasional </span></a></li>' +
              '<li><a style="text-decoration:none" href="#" data-action="gotoPage"><span>Laporan Pengelolaan SDM </span></a></li>'+
              '<li><a style="text-decoration:none" href="#" data-action="gotoPage"><span>Laporan Pengelolaan Teknologi Informasi </span></a></li>'+
              '<li><a style="text-decoration:none" href="#" data-action="gotoPage"><span>Analisis dan Pembahasan Managemen </span></a></li>',
          )
  }
  function shareModal() {
    var loading =
      '<div id="loadingIcon" class="fa-3x">' +
      '<i class="fas fa-circle-notch fa-spin"></i>' +
      '</div>'
    $('#modalShare .content').html(loading)
  
    var reportnames = []
    firebase.auth().onAuthStateChanged((user) => {
      if (user) {
        const urlAccount = 'https://yearly-api-prod.herokuapp.com/api/v1/accounts/' + user.uid
        const urlBuilder =
          'https://yearly-api-prod.herokuapp.com/api/v1/account_reports/' +
          getParam('rid') +
          '?firebase_id=' +
          user.uid
        //const urlAccountReports = 'https://yearly-api-prod.herokuapp.com/api/v1/accounts/'+user.uid+'/get_account_reports';
        user.getIdToken(true).then((idToken) => {
          axios
            .get(urlAccount, {
              responseType: 'json',
              headers: { Authorization: idToken },
            })
            .then((json) => {
              var username = json.data.username
              //console.log('user data '+ JSON.stringify(json.data))
            })
  
          //axios.get(urlAccountReports, {
          axios
            .get(urlBuilder, {
              responseType: 'json',
              headers: { Authorization: idToken },
            })
            .then((json) => {
              var username = json.data.report.username
              //console.log('From startPublishing username '+username)
              console.log('From startPublishing reports' + JSON.stringify(json.data))
              /*$.each(json.data.reports, function( k ){
                              //console.log('From startPublishing each k '+JSON.stringify(k))
                              reportnames.push( k );
                          });*/
  
              var title = json.data.report.report_slug
              console.log(title)
              debugger
              if (title != undefined) {
                var reportTitle = title
                var reportURL =
                  'https://yearly.report/from/' + json.data.report.username + '/' + title
                var simpleReportURL = 'yearly.report/from/' + json.data.report.username + '/' + title
              } else {
                var reportTitle = $('[data-field="reportTitle"]').text() //$("#titleReport").text();
                reportTitle = reportTitle.replace(/ /g, '-').toLowerCase()
  
                if (reportnames.includes(reportTitle)) {
                  reportTitle = reportTitle + '-' + Math.random().toString(36).substr(2, 4)
                }
                var reportURL =
                  'https://yearly.report/from/' +
                  json.data.report.username.username +
                  '/' +
                  reportTitle
                var simpleReportURL =
                  'yearly.report/from/' + json.data.report.username.username + '/' + reportTitle
              }
  
              /*var reportTitle = $('[data-field="reportTitle"]').text() //$("#titleReport").text();
                          reportTitle = reportTitle.replace(/ /g, '-').toLowerCase();
                          
                          if( reportnames.includes( reportTitle ) ){
                              reportTitle = reportTitle+'-'+Math.random().toString(36).substr(2, 4);
                          }
                          var reportURL = 'https://yearly.report/from/'+json.data.report.username+'/'+reportTitle;
                          var simpleReportURL = 'yearly.report/from/'+json.data.report.username+'/'+reportTitle;
  */
  
              // var userName = username.replace(/ /g, "");
  
              var userLog = localStorage.getItem('UserLogged')
              var userLogData = JSON.parse(userLog)
              //console.log('shareModal '+ JSON.stringify(userLogData))
              //console.log('shareModal '+ JSON.stringify(json.data))
              if (json.data.report.subscription_status == 'paid') {
                var shareContent =
                  '<div class="craftLink share">' +
                  '<h4>Report Link</h4>' +
                  '<a href="' +
                  reportURL +
                  '" target="_blank" class="url"><i class="fa fa-lock"></i>' +
                  reportURL +
                  '</a><br><br>' +
                  '<div>' +
                  '<div>' +
                  '<h4>Embed Report</h4>' +
                  '<textarea readonly>' +
                  '<iframe src="' +
                  reportURL +
                  '" style="position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; border: none; z-index: 9999;"></iframe>' +
                  '</textarea>' +
                  '</div>' +
                  '<br><br>' +
                  '<div>' +
                  '<h4>Report Widget</h4>' +
                  '<div style="padding-top: 60%; position: relative;"><iframe src="http://yearly.report/embed/?reportURL=' +
                  encodeURI(simpleReportURL) +
                  '&username=' +
                  username +
                  '&title=' +
                  reportTitle +
                  '" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; border: none;"></iframe></div>' +
                  '<br>' +
                  '<textarea readonly>' +
                  '<div style="padding-top: 60%; position: relative;"><iframe src="http://yearly.report/embed/?reportURL=' +
                  encodeURI(simpleReportURL) +
                  '&username=' +
                  username +
                  '&title=' +
                  reportTitle +
                  '" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; border: none;"></iframe></div>' +
                  '</textarea>' +
                  '</div>' +
                  '</div>' +
                  '</div>'
              }
              if (json.data.report.subscription_status == 'free') {
                var shareContent =
                  '<div class="craftLink share">' +
                  '<h4>Report Link</h4>' +
                  '<a href="' +
                  reportURL +
                  '" target="_blank" class="url"><i class="fa fa-lock"></i>' +
                  reportURL +
                  '</a><br><br>' +
                  '<div>' +
                  '<br><br>' +
                  '</div>' +
                  '</div>'
              }
              $('#modalShare .content').html(shareContent)
              $(this)
                .find('iframe')
                .attr(
                  'src',
                  'http://yearly.report/embed/?reportURL=' +
                    encodeURI(simpleReportURL) +
                    '&username=' +
                    username +
                    '&title=' +
                    reportTitle,
                )
              // check report name
              $('.craftLink button').attr('disabled', true)
              var preferredReportname = $('[data-field="reportname"]').text()
              var preferredReportusername = $('[data-field="reportusername"]').text()
              if (reportnames.includes(preferredReportname)) {
                $('[data-field="reportname"]').removeClass('valid')
                $('.craftLink button').attr('disabled', true)
                $('.craftLink .message').html(
                  'You already have a report that exists with this label.',
                )
                $('.craftLink .message').removeClass('hidden')
              } // end if prefeerredReportname
  
              if (
                /^[a-zA-Z0-9-_ ]*$/.test(preferredReportname) == false ||
                /^[a-zA-Z0-9-_ ]*$/.test(preferredReportusername) == false
              ) {
                //$('[data-field="reportname"]').removeClass('valid');
  
                if (/^[a-zA-Z0-9-_ ]*$/.test(preferredReportname) == false) {
                  $('[data-field="reportname"]').removeClass('valid')
                  //$('[data-field="reportusername"]').addClass('error');
                }
                if (/^[a-zA-Z0-9-_ ]*$/.test(preferredReportusername) == false) {
                  $('[data-field="reportusername"]').removeClass('valid')
                  $('[data-field="reportusername"]').addClass('error')
                }
  
                $('.craftLink button').attr('disabled', true)
                $('.craftLink .message').html('No special characters allowed. Dashes are fine.')
                $('.craftLink .message').removeClass('hidden')
              } else {
                $('[data-field="reportname"]').addClass('valid')
                $('.craftLink button').attr('disabled', false)
                $('.craftLink .message').addClass('hidden')
              }
            })
        })
      } //end of if user
    }) //end of fire Auth
  }
    </script>
</body>

</html>